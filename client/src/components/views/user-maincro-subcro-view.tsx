...
import { UserMaincroSubcroView } from "@shared/schema";
import { Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from "@/components/ui/select";
import { Button } from "@/components/ui/button";
import { Loader2, FilterX } from "lucide-react";

export function UserMaincroSubcroViewComponent() {
  ...
  const {
    ...
    queryKey: ["/api/user-view", maincroFilter, subcroFilter],
    queryFn: async () => {
      const queryParams = new URLSearchParams();
      if (maincroFilter) queryParams.append("maincro", maincroFilter);
      if (subcroFilter) queryParams.append("subcro", subcroFilter);
      
      const res = await fetch(`/api/user-view?${queryParams}`, { credentials: "include" });
      if (!res.ok) {
        throw new Error(`Error fetching user view: ${res.statusText}`);
      }
      return res.json();
    },
  });
  ...
  return (
    <>
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-2xl font-bold">User MainCro SubCro View</h1>
      </div>
      ...
      {isLoadingView ? (
        ...
        <DataTable
          ...
          primaryKey="id"
          tableName="User MainCro SubCro View"
        />
      )}
    </>
  );
